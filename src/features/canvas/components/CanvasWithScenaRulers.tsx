...
interface CanvasWithScenaRulersProps {
  showRulers?: boolean;
}

const MM_TO_PX = 3.7795275591; // 1mm = 3.7795275591px at 96 DPI

export const CanvasWithScenaRulers: React.FC<CanvasWithScenaRulersProps> = ({ 
  showRulers = true 
}) => {
  ...
  useEffect(() => {
    const updateSize = () => {
      const container = document.getElementById('canvas-with-rulers-container');
      if (container) {
  ...
  return (
    <div className="relative w-full h-full bg-slate-100 overflow-hidden" id="canvas-with-rulers-container">
      {showRulers && (
        <>
          {/* Corner */}
          <div 
